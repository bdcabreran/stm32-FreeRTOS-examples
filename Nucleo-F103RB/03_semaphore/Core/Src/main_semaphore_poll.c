...
void print_startup_msg(void)
{
	...
	log_message(LOG_LEVEL_INFO,"***** Theme : Semaphore RTOs Wasting Cycles ******\r\n");
	log_message(LOG_LEVEL_INFO,"************************************\r\n");
}
...
void task_a(void *arguments)
{
    ...
    while (1)
    {
      if(++count >= 5)
      {
        ...
        SEGGER_SYSVIEW_PrintfHost("sets flag\n");
        flag = 1; // set flag to 1 to "signal" task b to run 
      }
...
void task_b(void *arguments)
{
    ...
    while (1)
    {
      SEGGER_SYSVIEW_PrintfHost("starts polling flag\n");

      // repeateadly poll on flag.  As soon as it is non-zero,
      // blink the blue LED 3 times
      while (!flag);

      SEGGER_SYSVIEW_PrintfHost("flag received\n");
      flag = 0;
